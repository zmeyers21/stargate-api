...
namespace StargateAPI.Business.Commands
{
    ...
    public class CreateAstronautDutyPreProcessor : IRequestPreProcessor<CreateAstronautDuty>
    {
        ...
        {
            // Verify the person exists before we try to add a duty to them
            var person = _context.People.AsNoTracking().FirstOrDefault(z => z.Name == request.Name);

            if (person is null) throw new BadHttpRequestException("Bad Request");

            // If record exists with same title and start date, reject it
    ...
    public class CreateAstronautDutyHandler : IRequestHandler<CreateAstronautDuty, CreateAstronautDutyResult>
    {
        ...
        public async Task<CreateAstronautDutyResult> Handle(CreateAstronautDuty request, CancellationToken cancellationToken)
        {
            ...
            var person2 = await _context.People.Where(x =>  x.Name == request.Name).FirstOrDefaultAsync();

            // TODO: If person is not found here, we need to throw an error
